...
def cmd_scroll(args):
    if not (getattr(args, "mirror", False) or getattr(args, "background", False)):
        print("Error: 'scroll' requires --mirror or --background.", file=sys.stderr)
        sys.exit(1)
    with _make_client(args) as client:
...
def main():
    ...
    p = sub.add_parser("scroll", help="Scroll content via the scroll wheel (mirror/background modes)")
